...
import PROFILE_PIC from '../assets/kartoon.png';
  ...
  return (
    <section className='max-w-screen-xl px-6 mx-auto py-10 md:py-24' id='about'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 md:gap-4'>
        <img className='w-full h-80 object-cover rounded-lg mb-4 md:mb-0'
             src={PROFILE_PIC} alt='profile pic' />

        <div className='col-span-2 bg-gradient-to-br from-blue-950 to-slate-900 rounded-lg border border-cyan-500/20 p-6'>
          <p className='text-cyan-100 text-xs md:text-[13px] font-normal text-justify leading-6'>
...
const InfoCard = ({ lable, count }) => {
  return (
    <div className='bg-gradient-to-br from-blue-950 to-slate-900 rounded-lg border border-cyan-800/40 px-4 md:px-6 py-3'>
      <h6 className='text-primary text-xl md:text-2xl font-semibold'>{count}</h6>

      <span className='text-cyan-300 text-xs md:text-sm font-normal'>{lable}</span>
    </div>
  );
};
